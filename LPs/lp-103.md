...
This proposal introduces the Linear Secret Sharing Scheme (LSS) implementation in Lux Network's threshold cryptography suite. LSS is a foundational threshold signature protocol that enables dynamic resharing - the ability to change the participant set and threshold without changing the public key. This capability is crucial for long-lived systems where validator sets change over time. LSS complements the CGG21 (CMP) protocol described in LP-14 by providing a simpler, more flexible approach to threshold signatures with unique dynamic properties not available in ECDSA-based schemes.
...
1. Each party pᵢ:
   a. Generate random polynomial fᵢ(x) of degree t-1
   b. Compute shares sᵢⱼ = fᵢ(j) for all j
   c. Compute commitments Cᵢₖ = gᵏ·Gfᵢₖ for coefficients
   d. Send sᵢⱼ to party pⱼ encrypted

2. Each party pᵢ:
   a. Verify received shares using commitments
   b. Compute final share sᵢ = Σⱼ sⱼᵢ
   c. Compute public key Y from commitments
