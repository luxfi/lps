...
|-------|---------|------------------|----------------|
| **P-Chain** | Platform/Validators | ✅ Re-launch | Minimal (validators) |
| **C-Chain** | EVM Smart Contracts | ✅ Re-launch | **Full state migration** |
| **X-Chain** | Asset Exchange | ✅ Re-launch | Minimal (if any) |
...
**State preservation**: **Only C-Chain EVM state is migrated** in mainnet regenesis:
...
func (rc *RegensisCoordinator) ExportRegensisChains(epoch uint64) error {
    ...
    // Export only C-Chain state (primary migration)
    // P and X chains have minimal/no state migration
    cChainState := rc.chains["C"].ExportFullState(epoch)

    // P-Chain: Export validator set only
    pChainValidators := rc.chains["P"].ExportValidators(epoch)

    // X-Chain: Minimal state if needed
    xChainMinimal := rc.chains["X"].ExportMinimalState(epoch)

    return rc.verifyExports(cChainState, pChainValidators, xChainMinimal)
}
```
...
Based on Ethereum's genesis format and Avalanche's subnet migration patterns. Special thanks to the Lux Core Team for implementing mainnet regenesis (P, C, X chains) with C-Chain EVM state migration, and Q-Chain as a new quantum-resistant deployment.
